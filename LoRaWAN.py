...
def airtime(sf,cr,pl,bw):
    ...
    print("AIRTIME sf", sf, " cr", cr, "pl", pl, "bw", bw)
...
class myNode():
    ...
    def __repr__(self):
        return f'> NODE(node {self.nodeid}: x={self.x:.3f}, y={self.y:.3f}, dist={self.dist:.3f})'
...
class assignParameters():
    def __init__(self, nodeid, distance):
        ...
        print("ASSIGN Lpl:", Lpl)
        Prx = self.txpow - GL - Lpl
        minairtime = 9999
        minsf = 0
        minbw = 0
        print("ASSIGN Prx:", Prx)
        ...
        print("ASSIGN best sf:", minsf, " best bw: ", minbw, "best airtime:", minairtime)
        if (minsf != 0):
            self.rectime = minairtime
            self.sf = minsf

        # SF, BW, CR and PWR distributions
        print("ASSIGN bw", self.bw, "sf", self.sf, "cr", self.cr)
...
class myPacket():
    def __init__(self, nodeid, freq, sf, bw, cr, txpow, distance):
        ...
        print('PACKET txpow', self.txpow, 'GL', GL, 'Lpl', Lpl)
        self.rssi = self.txpow - GL - Lpl
        print("PACKET node id", self.nodeid, "symTime ", self.symTime, "rssi", self.rssi)
        self.rectime = airtime(self.sf,self.cr,self.pl,self.bw)
        print("PACKET rectime node ", self.nodeid, "  ", self.rectime)
