...
"""
 ...
 scenario when alpha=1
 author: Khaled Abdelfadeel khaled.abdelfadeel@mycit.ie
"""

"""
 SYNOPSIS:
   ./confirmabletdmaSFTX.py <nodes> <datasize> <avgsend> <collision> <randomseed>
 DESCRIPTION:
    nodes
...
class stage1():
    def __init__(self, nodeid, distance):
        ...
        print('PREV STAGE1 self.sf', self.sf)
        # balance the frame lengths
        print("minsf: ", minsf)
        global SFdistribution
        tempcost = 9999999999999
        tempsf = minsf
        for sf in range(minsf, 6):
            cost = max(SFdistribution[sf]+1, (1.0/Dutycycle))*airtime(sf+7, CodingRate, LorawanHeader+PcktLength_SF[sf], Bandwidth)*(math.ceil(float(datasize)/(PcktLength_SF[sf]*ChanlperSF[sf]))+(ChanlperSF[sf]-1))
            print("sf: ", sf, "cost: ", cost)
            if (cost < tempcost):
                tempcost = cost
                tempsf = sf

        print('PREV2 STAGE1 after A1 tempsf was ', minsf, 'and now is', tempsf)
        self.sf = tempsf+7
        self.rectime = airtime(self.sf, self.cr, LorawanHeader+PcktLength_SF[i], self.bw)

        print('PREV2-STAGE1 Prx:', Prx, ' ; self.sf:', self.sf, ' ; self.bw:', self.bw, ' ; TXPOW:', self.txpow)
        print('PREV3-STAGE1 sensi:', sensi[self.sf - 7, [125, 250, 500].index(self.bw) + 1])

        # control TX of SF8
        if (self.sf == 8):
            print('STAGE1 ------- SF=8, alternative:', self.txpow - math.floor(Prx - sensi[self.sf - 7, [125, 250, 500].index(self.bw) + 1]))
        ...
        global CRdistribution, TXdistribution, BWdistribution
        print('STAGE1 TXPOW:', self.txpow, "; LEN TXdist:", len(TXdistribution))
